...
After the visual examination of the data and key relationships I was curious about I dove a bit deeper and ran five regression models, shown in Table 1 below. All models are ordinary least squares regression models and the unit of analysis are individual runs. In the first column I investigate if my running distance has changed over time. There coefficient is for the *year* variable is positive and weakly (.1) statistically significant. There seems to be some increase in the length of my runs over time but the result is not statistically significant at [conventional levels](https://www.tandfonline.com/doi/pdf/10.1080/00031305.2016.1154108?needAccess=true). Columns 2 and 3 examine the factors that explain variability in my heart rate for each run. First, I look at the bivariate relationship between average heart rate and average pace. The relationship between the two variables is rather strong and highly statistically significant. For every additional minute that it takes me to run one kilometer my average heart rate goes down by 11.77 beats per minute. This associations between average heart rate and average pace remains basically unchanged even after I control for year, distance, whether the run was longer than 10k or not, and elevation gain of the run (Column 3). Something to be happy about: the coefficient for year is negative and statistically significant. As time advances I have become fitter! 

In column four I look at the factors that explain variation in my average pace. Two factors seems to play a role: 1) over time I have become faster and 2) as the distance increases my pace goes down. Whether or not the run is longer than 10k or not does (suprisingly) not affect my pace. Here the recovery runs appear to be at work. Lastly, in column 5 I take a look at the estimated calories that each run burned. What explains variation here? The average pace plays an important role: For every additional minute I need for one kilometer the estimated calories I burn go down by about 28. On the other hand, each additional meter in elevation that I gain burns an extra 1.04 calories. The overall distance I ran does not, after controlling for other factors, explain variation in the calories I burn during a run. However, the binary classification of the run into more or less than 10k shows that it is not necessarily the distance but the intensity of the run that explains how much energy is burned.
